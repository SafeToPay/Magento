...
Validation.add('validate-s2p-cc-number', 'Please enter a valid credit card number.', function(v, elm) {
    return true;
    //return Validation.get('validate-cc-number').test(v, elm) && Validation.get('validate-cc-type').test(v, elm);
});

Validation.add('validate-s2p-cc-exp', 'Incorrect credit card expiration date.', function(v, elm){
    var ccExpMonth   = v;
    var ccExpYear    = $(elm.id.substr(0,elm.id.indexOf('_expiration')) + '_expiration_yr').value;
    return ccExpMonth && ccExpYear && Validation.get('validate-cc-exp').test(v, elm);
});

Validation.add('validate-s2p-cc-cvn', 'Please enter a valid credit card verification number.', function(v, elm){
